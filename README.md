...
- **更新を取り込んだときはインストーラも自動で再実行**される(macOS/Linux は install.sh、
  Windows は PowerShell 経由で install.ps1)。リンク対象の追加や構成変更にも手動作業なしで追従する
...
(後述の SessionStart フック。更新時はインストーラの再実行まで自動で行われるため、
手動作業が必要なのは各マシンでの初回インストールだけです)。
...
2. 他のマシンには**次のセッション開始時に自動反映**される(SessionStart フックが pull し、
   更新があればインストーラも自動で再実行される)。すぐ反映したい場合のみ手動で `git pull`
